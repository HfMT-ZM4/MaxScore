...
function bang() {
  ...
  if ("event_length" in djsterAttributes && "pulse_length" in djsterAttributes) {
    ...
    var smaller = JSON.parse('{"new":"text","x":'+currentSize*89+',"y":'+currentSize*54+',"text-anchor":"end","font-family":"Arial","font-size":'+currentSize*8+',"style":{"fill":"'+colorPalette.event.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
    ...
    var larger = JSON.parse('{"new":"text","x":'+currentSize*114+',"y":'+currentSize*54+',"text-anchor":"end","font-family":"Arial","font-size":'+currentSize*8+',"style":{"fill":"'+colorPalette.event.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  var pitch_circle = JSON.parse('{"new":"ellipse","cx":23,"cy":44,"rx":5,"ry":5,"style":{"stroke":"'+colorPalette.pitch.dark[onoff]+'","stroke-width":1,"stroke-opacity":1},"transform":"'+transformMatrix+'"}');
  ...
  if ("melody_scope" in djsterAttributes) {
    var melody_scope = JSON.parse('{"new":"text","x":'+currentSize*28+',"y":'+currentSize*42+',"font-family":"Arial","font-size":'+currentSize*10+',"style":{"fill":"'+colorPalette.pitch.mid[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  if ("pitch_range" in djsterAttributes && "pitch_center" in djsterAttributes) {
    var pitch_max = JSON.parse('{"new":"text","x":'+currentSize*26+',"y":'+currentSize*26+',"font-family":"Arial","font-size":'+currentSize*10+',"style":{"fill":"'+colorPalette.pitch.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
    ...
    var pitch_min = JSON.parse('{"new":"text","x":'+currentSize*26+',"y":'+currentSize*68+',"font-family":"Arial","font-size":'+currentSize*10+',"style":{"fill":"'+colorPalette.pitch.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  if ("tonic_pitch" in djsterAttributes) {
    var tonic_pitch = JSON.parse('{"new":"text","x":'+currentSize*55+',"y":'+currentSize*55+',"text-anchor":"end","font-family":"Arial","font-size":'+currentSize*11+',"style":{"fill":"'+colorPalette.pitch.mid[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  if ("scale" in djsterAttributes) {
    ...
    var element = JSON.parse('{"new":"text","x":'+currentSize*5+',"y":'+currentSize*11+',"font-family":"Arial","font-size":'+currentSize*10+',"style":{"fill":"'+colorPalette.pitch.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  if ("dynamics" in djsterAttributes) {
    ...
    var fillbar = JSON.parse('{"new":"rect","x":5,"y":75,"width":'+fillbarlength+',"height":10,"style":{"fill":"'+colorPalette.dynamics.light[onoff]+'","fill-opacity":1},"transform":"'+transformMatrix+'"}');
    fillbar.id = groupId+"_dynamics_fillbar";
    outputPicster["picster-element"][0]["val"]["child"].push(fillbar);
    var border = JSON.parse('{"new":"rect","x":5,"y":75,"width":53,"height":10,"style":{"stroke":"'+colorPalette.dynamics.dark[onoff]+'","stroke-width":1, "stroke-opacity":1},"transform":"'+transformMatrix+'"}');
    border.id = groupId+"_dynamics_border";
    outputPicster["picster-element"][0]["val"]["child"].push(border);

    var text = JSON.parse('{"new":"text","x":'+currentSize*7+',"y":'+currentSize*83+',"child":"D","font-family":"Arial","font-size":'+currentSize*8+',"style":{"fill":"'+colorPalette.dynamics.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  if ("attenuation" in djsterAttributes) {
    ...
    var fillbar = JSON.parse('{"new":"rect","x":62,"y":75,"width":'+fillbarlength+',"height":10,"style":{"fill":"'+colorPalette.dynamics.light[onoff]+'","fill-opacity":1},"transform":"'+transformMatrix+'"}');
    fillbar.id = groupId+"_attenuation_fillbar";
    outputPicster["picster-element"][0]["val"]["child"].push(fillbar);

    var border = JSON.parse('{"new":"rect","x":63,"y":75,"width":53,"height":10,"style":{"stroke":"'+colorPalette.dynamics.dark[onoff]+'","stroke-width":1, "stroke-opacity":1},"transform":"'+transformMatrix+'"}');
    border.id = groupId+"_attenuation_border";
    outputPicster["picster-element"][0]["val"]["child"].push(border);

    var text = JSON.parse('{"new":"text","x":'+currentSize*64+',"y":'+currentSize*83+',"child":"A","font-family":"Arial","font-size":'+currentSize*8+',"style":{"fill":"'+colorPalette.dynamics.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  if ("eventfulness" in djsterAttributes) {
    ...
    var fillbar = JSON.parse('{"new":"rect","x":63,"y":61,"width":'+fillbarlength+',"height":10,"style":{"fill":"'+colorPalette.event.light[onoff]+'","fill-opacity":1},"transform":"'+transformMatrix+'"}');
    fillbar.id = groupId+"_eventfulness_fillbar";
    outputPicster["picster-element"][0]["val"]["child"].push(fillbar);

    var border = JSON.parse('{"new":"rect","x":63,"y":61,"width":53,"height":10,"style":{"stroke":"'+colorPalette.event.dark[onoff]+'","stroke-width":1, "stroke-opacity":1},"transform":"'+transformMatrix+'"}');
    border.id = groupId+"_eventfulness_border";
    outputPicster["picster-element"][0]["val"]["child"].push(border);

    var text = JSON.parse('{"new":"text","x":'+currentSize*64+',"y":'+currentSize*69+',"child":"E","font-family":"Arial","font-size":'+currentSize*8+',"style":{"fill":"'+colorPalette.event.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  if ("harmoniclarity" in djsterAttributes) {
    ...
    var fillbar = JSON.parse('{"new":"rect","x":5,"y":'+fillbarystart+',"width":10,"height":'+fillbarlength+',"style":{"fill":"'+colorPalette.pitch.light[onoff]+'","fill-opacity":1},"transform":"'+transformMatrix+'"}');
    fillbar.id = groupId+"_harmoniclarity_fillbar";
    outputPicster["picster-element"][0]["val"]["child"].push(fillbar);

    var border = JSON.parse('{"new":"rect","x":5,"y":18,"width":10,"height":53,"style":{"stroke":"'+colorPalette.pitch.dark[onoff]+'","stroke-width":1, "stroke-opacity":1},"transform":"'+transformMatrix+'"}');
    border.id = groupId+"_harmoniclarity_border";
    outputPicster["picster-element"][0]["val"]["child"].push(border);

    var text = JSON.parse('{"new":"text","x":'+currentSize*7+',"y":'+currentSize*67+',"child":"H","font-family":"Arial","font-size":'+currentSize*8+',"style":{"fill":"'+colorPalette.pitch.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
  ...
  if ("metriclarity" in djsterAttributes) {
    ...
    var fillbar = JSON.parse('{"new":"rect","x":63,"y":3,"width":'+fillbarlength+',"height":10,"style":{"fill":"'+colorPalette.meter.light[onoff]+'","fill-opacity":1},"transform":"'+transformMatrix+'"}');
    fillbar.id = groupId+"_metriclarity_fillbar";
    outputPicster["picster-element"][0]["val"]["child"].push(fillbar);

    var border = JSON.parse('{"new":"rect","x":63,"y":3,"width":53,"height":10,"style":{"stroke":"'+colorPalette.meter.dark[onoff]+'","stroke-width":1, "stroke-opacity":1},"transform":"'+transformMatrix+'"}');
    border.id = groupId+"_metriclarity_border";
    outputPicster["picster-element"][0]["val"]["child"].push(border);

    var text = JSON.parse('{"new":"text","x":'+currentSize*64+',"y":'+currentSize*11+',"child":"M","font-family":"Arial","font-size":'+currentSize*8+',"style":{"fill":"'+colorPalette.meter.dark[onoff]+'","fill-opacity":1},"transform":"matrix(1,0,0,1,0,0)"}');
